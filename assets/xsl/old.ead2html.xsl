			...
			<xsl:choose>
				...
				<xsl:when test="@level='file' or @level='item' or (@level='otherlevel'and child::did/container)">
					...
					<tr class="{$colorClass}" id="{@id}">
						<!-- Containers -->
						<xsl:variable name="parent_id" select="did/container[@id][1]/@id" />
						<!-- Cell 1-->
						<xsl:choose> 
							<xsl:when test="string-length($container)">							
								<td class="container"><xsl:value-of select="$container"/><xsl:text>:</xsl:text> <xsl:value-of select="did/container[@type = $container][1]"/></td>
							</xsl:when>
							<xsl:otherwise><td></td></xsl:otherwise>
						</xsl:choose>
						<!-- Cell 2 -->
						<xsl:choose>
							<xsl:when test="string-length($container2)">
								<td class="container"><xsl:value-of select="$container2"/><xsl:text>:</xsl:text> <xsl:value-of select="did/container[@type = $container2][1]"/>
							             <xsl:if test="$count_type &gt; 1">
									<xsl:text>&#160;(</xsl:text><xsl:value-of select="did/container[1]/@label" /><xsl:text>)</xsl:text>
									</xsl:if>					
								</td>
							</xsl:when>
							<xsl:otherwise><td></td></xsl:otherwise>
						...
						<!-- Cell 3-->
						<xsl:choose>
							<xsl:when test="string-length($container3) and did/container[@type = $container3 and @parent = $parent_id]">
								<td class="container"><xsl:value-of select="$container3"/><xsl:text>:</xsl:text> <xsl:value-of select="did/container[@type = $container3][1]"/></td>
							</xsl:when>
							<xsl:otherwise><td></td></xsl:otherwise>
						</xsl:choose>
					...
					<!-- Extra rows for a 3 and 4 container-->
					<xsl:if test="string-length($container3) and string-length($container4)">
						<tr class="{$colorClass}">
							<xsl:choose>
								<xsl:when test="$container3 = 'Box' and $container4 = 'Folder'">
									<td class="container"><xsl:value-of select="$container3"/><xsl:text>:</xsl:text> <xsl:value-of select="did/container[3]"/></td>
									<td class="container"><xsl:value-of select="$container4"/><xsl:text>:</xsl:text> <xsl:value-of select="did/container[4]"/>
										<xsl:if test="$count_type &gt; 1">
											<xsl:text>&#160;(</xsl:text><xsl:value-of select="did/container[3]/@label" /><xsl:text>)</xsl:text>
										</xsl:if>
									</td>
								</xsl:when>
								<xsl:when test="$container4 = 'Box' and $container3 = 'Folder'">
									<td class="container"><xsl:value-of select="$container4"/><xsl:text>:</xsl:text> <xsl:value-of select="did/container[4]"/></td>
									<td class="container"><xsl:value-of select="$container3"/><xsl:text>:</xsl:text> <xsl:value-of select="did/container[3]"/></td>
								</xsl:when>
							</xsl:choose>
							<td colspan="3"></td>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
			   <xsl:if test="count(../dao) = 1"> 
				<p><a class="daoLink" href="{concat(@ns2:href,'?urlappend=/mode/embed')}"  title="{@ns2:role}">Please click for image:<xsl:value-of select="@ns2:title"/></a></p>
		          </xsl:if>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
