...
sub publishedFiles {
  ...
  if (defined($pjax)) {
    $body .= showPublished($identifier, $collections{$identifier});
  }
    ...
    for (keys %collections) {
      ...
      $collections_tab .= '<li class="archive-' . $_ . ($_ eq $identifier ? ' selected' : '') . '"><a href="#' . $_ . '" class="tab archive-' . $_ . '" data-id="' . $_ . '" data-upload="' . $confHash{'PUBLISHER_URI'} . '/upload/' . $_ .'" data-name="' . $collections{$_} . '" data-uri="' . $confHash{'PUBLISHER_URI'} . '/published/' . $_ . '">' . $collections{$_} . '</a></li>';
